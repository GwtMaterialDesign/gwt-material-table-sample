...
if [ ! -f $TRAVIS_BUILD_DIR/target/gwt-material-table-sample-*.war ]; then
...
cd $HOME
rm -rf gh-pages
git clone --quiet --branch=gh-pages https://$GH_TOKEN@github.com/GwtMaterialDesign/gwt-material-table-sample gh-pages > /dev/null
cd gh-pages

# remove the GwtMaterialDemo directories from git.
if [[ -d ./snapshot/gmdtablesample ]]; then
git rm -rf ./snapshot/gmdtablesample
fi
...
unzip -u $TRAVIS_BUILD_DIR/target/gwt-material-table-sample-*.war -d ./snapshot/
